      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div style={{
          ...
          <div style={{
            ...
            <button
              onClick={() => { setWeekOffset(Math.max(0, weekOffset - 1)); setSelectedDay(null); }}
            ...
            <button
              onClick={() => { setWeekOffset(Math.min(7, weekOffset + 1)); setSelectedDay(null); }}
          ...
          <div style={{
            ...
            {visibleDays.map((date) => {
              ...
              const slots = availDoc && Array.isArray(availDoc.slots) ? availDoc.slots : [];
              const bookedCount = getBookedSlots(format(date, 'yyyy-MM-dd')).length;
              const hasSlots = slots.length > 0;
              const isSel = selectedDay && format(date, 'yyyy-MM-dd') === format(selectedDay, 'yyyy-MM-dd');
              ...
              return (
                <motion.button
                  ...
                  style={{
                    ...
                    gap: '3px',
                    background: isSel
                      ? 'linear-gradient(135deg, rgba(201,168,76,0.35), rgba(232,201,122,0.2))'
                      : hasSlots
            ...
            {selectedDay && (
              ...
              >
                <div style={{
                  ...
                  >
                    {(() => {
                      ...
                      const currentSlots = availDoc && Array.isArray(availDoc.slots) ? availDoc.slots : [];
                      const booked = getBookedSlots(dateStr);
                      const freeSlots = TIME_SLOTS.filter(s => !booked.includes(s));
                      ...
                      <motion.button
                        ...
                        style={{
                          ...
                          background: isBooked
                            ? 'rgba(231,76,60,0.12)'
                            : isActive
                              ? 'linear-gradient(135deg, rgba(201,168,76,0.25), rgba(232,201,122,0.12))'
                ...
                <div style={{ marginTop: '20px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                  {[
                    { color: 'rgba(201,168,76,0.7)', bg: 'rgba(201,168,76,0.25)', label: 'Disponible' },
                    { color: 'rgba(255,255,255,0.08)', bg: 'rgba(255,255,255,0.03)', label: 'Fermé' },
                    { color: 'rgba(231,76,60,0.5)', bg: 'rgba(231,76,60,0.12)', label: 'Déjà réservé' },
                  ].map(item => (
                    <div key={item.label} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <span style={{ fontFamily: 'Jost, sans-serif', fontSize: '11px', color: '#8A7968' }}>
                        {item.label}
                      </span>
                    </div>
                  ))}
