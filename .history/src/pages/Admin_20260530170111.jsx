...
const ALL_SLOTS = ['08:00','09:00','10:00','11:00','13:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00','21:00','22:00','23:00'];
...
const STATUS_NORMALIZE = {
  ...
  paye_entierement:    'waiting_confirmation',
  expire:              'expired',
  annule:              'cancelled',
...
function isInactive(b) {
  return INACTIVE_STATUSES.includes(getEffectiveStatus(b));
}
...
function EditBookingModal({ booking, bookings, availability, onClose, onSave }) {
  ...
  const getBookedSlotsForDate = (d) =>
    bookings.filter(b => b.date === d && b.id !== booking.id && !isInactive(b)).map(b => b.time);
...
export default function Admin() {
  ...
  // ─── Auto-complétion des RDV passés ─────────────────────────────────────────
  useEffect(() => {
  ...
  // ─── Auto-suppression des archivés anciens ────────────────────────────────────
  useEffect(() => {
    const toDelete = bookings.filter(b => {
      if (getEffectiveStatus(b) !== 'archived') return false;
  ...
  useEffect(() => {
    const now = new Date();
    bookings.filter(b => getEffectiveStatus(b) === 'confirmed' && !b.reminderSent).forEach(async b => {
      try {
        const dt = parseISO(`${b.date}T${b.time}:00`);
  ...
  const handleEditSave = async (form) => {
    ...
    if (form.date !== b.date || form.time !== b.time) {
      const conflicting = bookings.find(x =>
        x.id !== b.id && x.date === form.date && x.time === form.time && !isInactive(x)
      );
    ...
    await updateDoc(doc(db, 'bookings', b.id), {
      ...
      acompte: montantPayeNum,
      status: form.status,
      statutReservation: form.status,
      updatedAt: serverTimestamp(),
    });
    const changes = {};
    const prevStatus = getEffectiveStatus(b);
    if (form.date !== prevDate) changes.date = `${prevDate} → ${form.date}`;
    if (form.time !== prevTime) changes.time = `${prevTime} → ${form.time}`;
    if (form.status !== prevStatus) changes.statut = `${STATUS_LABEL[prevStatus]} → ${STATUS_LABEL[form.status]}`;
  ...
  const pendingPayment = bookings.filter(b => getEffectiveStatus(b) === 'pending_payment').length;

  const totalEncaisse = bookings
    .filter(b => !['cancelled', 'expired', 'archived'].includes(getEffectiveStatus(b)))
    .reduce((acc, b) => acc + getMontantPaye(b), 0);

  const dueSoonReminders = bookings.filter(b => {
    if (getEffectiveStatus(b) !== 'confirmed') return false;
  ...
  const bulkArchiveOld = async () => {
    ...
    const toArchive = bookings.filter(b => ['cancelled', 'expired'].includes(getEffectiveStatus(b)));
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '3px', marginBottom: '20px' }}>
                    {calendarDays.map((date) => {
                      ...
                      const dayBookings = bookings.filter(b => b.date === dateStr && !isInactive(b));
                  ...
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '8px', padding: '18px 14px', marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                      {ALL_SLOTS.map((slot) => {
                        ...
                        const bookingForSlot = bookings.find(b => b.date === editingDate && b.time === slot && !isInactive(b));
                        return (
      ...
      <AnimatePresence>
        {proofViewer && (
          <motion.div
            ...
            style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 9999 }}>
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              style={{ width: '100%', maxWidth: '500px', background: '#111', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid rgba(201,168,76,0.15)', background: 'linear-gradient(135deg, #111, #1a1a1a)' }}>
              ...
              <img src={proofViewer} alt="Preuve de paiement" style={{ width: '100%', maxHeight: '75vh', objectFit: 'contain', display: 'block', background: '#000' }} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
